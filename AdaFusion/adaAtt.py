...
from AdaFusion.module import Attention, PreNorm, FeedForward, Upsample


class Tem_Transformer(nn.Module):
    def __init__(self, dims, depth, heads, dim_head, dropout=0.):
        super().__init__()
...
class Spa_Transformer(nn.Module):
    def __init__(self, dims, depth, heads, dim_head, dropout=0.):
        super().__init__()
        self.layers = nn.ModuleList([])
        self.norm = nn.LayerNorm(dims[-1])
...
class AdaAttetion(nn.Module):
    def __init__(self, image_size, patch_size, num_frames, depth=3, heads=3,
        ...
        self.temporal_transformer = Tem_Transformer(temporal_dims, depth, heads, dim_head, dropout)
        self.space_transformer = Spa_Transformer(space_dims, depth+1, heads, dim_head, dropout)
